...
protected:
    void joy_callback(sensor_msgs::msg::Joy::SharedPtr msg) {
        if ((joystick_button_ < int(msg->buttons.size())) && (msg->buttons[joystick_button_])) {
            ...
            if ((now - last_joy_time_).seconds() < 0.5) return;
            last_joy_time_ = now;
            learning_ = !learning_;
            if (learning_) {
    ...
    void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr & img_msg) {
        if (!learning_) return;
        if ((rclcpp::Time(img_msg->header.stamp) - last_command_time_).seconds() > 0.1) return;
        ...
        if ((hypot(last_pose.x - new_pose.x, last_pose.y - new_pose.y) < min_displacement_) &&
            (fabs(remainder(last_pose.theta - new_pose.theta, 2 * M_PI)) < min_rotation_)) return;

        last_pose = new_pose;

        cv::Mat img(cv_bridge::toCvShare(img_msg, "bgr8")->image);
        int label = 1;
        if (last_command_.linear.x < -rotation_threshold_) label = 0;
        else if (last_command_.linear.x > rotation_threshold_) label = 2;

        bool save_it = type_counter_[label] < (unsigned)max_image_per_type_;
        if (save_it) {
...
public:
    ShoreFollowerObserve() : rclcpp::Node("shore_follower_observe") {
        ...
        this->declare_parameter("~/rotation_threshold", 0.4);
        ...
        last_pose.y = transformStamped.transform.translation