...
class DiversifiedDriver(Node):
    ...
    def timer_cb(self):
        ...
        phase = int(t // 10) % 6  # Each phase lasts 5 seconds

        if phase == 0:
            # Phase 0: Constant velocity (slow forward)
            twist.linear.x = 0.0

        elif phase == 1:
            # Phase 1: Constant velocity (slow forward)
            twist.linear.x = -0.3

        elif phase == 2:
            # Phase 2: Constant velocity (slow backward)
            twist.linear.x = 0.3

        elif phase == 3:
            # Phase 3: Sine wave (smooth oscillation)
            twist.linear.x = sin(2 * pi * 0.4 * t)  # frequency 0.2 Hz

        elif phase == 4:
            # Phase 4: Slow ramp transition between -1 and 1
            twist.linear.x = ((t % 5) / 5.0) * 2.0 - 1.0  # ramp -1 → +1

        elif phase == 5:
            # Phase 5: Short bursts
